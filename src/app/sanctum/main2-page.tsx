...
import { Loader2, Lock, Search, Shield, Trash2, AlertCircle, Eye, EyeOff } from 'lucide-react';
// import { Loader2, Shield, Eye, EyeOff, Search, Plus, Lock, AlertCircle, Trash2 } from 'lucide-react';
...
// ── Constants ──────────────────────────────────────────────────────────────────
const INACTIVITY_MS = 213_000; // 60 seconds
...
export default function SanctumPage() {
  ...
  // ── Detail view (read-only project open) ────────────────────────────────────
  const [detailProject, setDetailProject] = useState<SanctumProject | null>(null);

  // ── Inactivity auto-lock ─────────────────────────────────────────────────────
  const lockTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const [lockCountdown, setLockCountdown] = useState(INACTIVITY_MS / 1000);
  ...
  const handleUnlocked = (space: SanctumSpace, password: string) => {
    ...
    // Refresh settings so activation badge updates
  ...
  if (authLoading || settingsLoading) {
    return (
      <div className="min-h-screen bg-[#0B0E14] flex items-center justify-center text-[#94A3B8]">
        ...
        <span className="text-sm">Loading Sanctum…</span>
      </div>
    );
  }
  ...
  // ── Detail modal ─────────────────────────────────────────────────────────────
  // Opens a full-screen view for reading a project (no editing inline, use Edit button)
  const DetailView = ({ project }: { project: SanctumProject }) => (
    <div className="fixed inset-0 z-[600] bg-[#0B0E14] flex flex-col overflow-hidden">
      <header className="sticky top-0 z-10 bg-[#151922]/95 backdrop-blur
        border-b border-white/10 px-4 sm:px-8 py-4 flex items-center gap-3">
        <button
        ...
        <h1 className="flex-1 font-semibold text-[#E2E8F0] text-base truncate">{project.title}</h1>
        <button
        ...
        <button
          onClick={() => { setDetailProject(null); setDeleteModal({ open: true, id: project.id, name: project.title }); }}
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-8 max-w-4xl mx-auto w-full">
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: project.content || '<p class="text-[#94A3B8]">No content yet.</p>' }}
        />
      </div>
    </div>
  );
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-[#0B0E14] w-full relative overflow-y-auto">
        ...
        {!session ? (
          ...
          <>
            {/* ── Unlocked content ── */}
            <div className="px-4 sm:px-8 py-8 mt-8 md:mt-0">

              {/* ── Desktop-only slim top bar: lock counter + lock button ── */}
              {/* On mobile these controls live inside SanctumHero to avoid overlap */}
              <div className="flex items-center justify-end gap-2 mb-4">
                ...
                <button
                  onClick={() => { setSession(null); setProjects([]); }}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-[#94A3B8]
              ...
              <SanctumHero
                ...
                onLock={() => { setSession(null); setProjects([]); }}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                  {filtered.map((p) => (
                    <SanctumCard
                      ...
                      onOpen={(proj) => setDetailProject(proj)}
                    />
                  ))}
                </div>
              )}
            ...
            {/* ── Detail view overlay ── */}
            {detailProject && <DetailView project={detailProject} />}
          </>
        )}
