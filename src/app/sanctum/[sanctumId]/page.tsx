...
// Editor is browser-only
const RichTextEditor = dynamic(() => import('@/components/editor/RichTextEditor'), {
  ssr: false,
  loading: () => (
    <CuplusLoader fullScreen label="Loading editor…" />
  ),
});
...
// ── Password gate (same style as main Sanctum page) ───────────────────────────
interface GateProps {
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function SanctumProjectPage() {
  ...
  // ── Session (space + password — memory only, cleared when tab closes) ──────────
  // On load: check sessionStorage first. If the user already unlocked a space on
  // the main Sanctum page in this tab, skip the gate entirely.
  ...
  // ── Editor stable seed (prevents reverse-write bug) ──────────────────────────
  const [editorKey, setEditorKey]                       = useState(0);
  const [editorInitialContent, setEditorInitialContent] = useState('');

  // ── Load project once session is unlocked ─────────────────────────────────────
  useEffect(() => {
    if (!session || !sanctumId || !userId) return;
    setLoading(true);
    // We fetch all projects for the space then find by id, since there's no
    // single-doc decrypt function (each doc needs password to decrypt).
