...
export default function RichTextEditor({
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    // ── Ctrl+Q: cycle block format p → h1 → h2 → h3 → blockquote → p ─────────
    if ((e.ctrlKey || e.metaKey) && e.key === 'q') {
      e.preventDefault();
      const CYCLE: string[] = ['p', 'h1', 'h2', 'h3', 'blockquote'];

      // Detect the block-level tag surrounding the current cursor/selection.
      // Walk up from the anchor node until we find a block element inside the editor.
      const sel = window.getSelection();
      let current = 'p'; // default assumption
      if (sel && sel.rangeCount > 0) {
        let node: Node | null = sel.getRangeAt(0).commonAncestorContainer;
        // If text node, start from its parent
        if (node && node.nodeType === Node.TEXT_NODE) node = node.parentElement;
        while (node && node !== editorRef.current) {
          const tag = (node as HTMLElement).tagName?.toLowerCase();
          if (tag && CYCLE.includes(tag)) { current = tag; break; }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="editor-body flex-1 min-h-0 overflow-y-auto outline-none text-sm leading-7 p-5"
        style={{ color: 'var(--text-main)', background: 'var(--bg)' }}
      />
